...
`pathviewr` defines trajectories as continuous movement from one side of
a region of interest to the other. Before trajectories can be defined,
the region of interest must be selected via
[`select_x_percent()`](https://docs.ropensci.org/pathviewr/reference/select_x_percent.md)
in addition to all the previous steps of the data import and cleaning
pipeline as described in [the Data Import and Cleaning
vignette](https://docs.ropensci.org/pathviewr/articles/data-import-cleaning.html).
We’ll start with loading `pathviewr` and a few packages from the
...
`pathviewr`’s all-in-one cleaning functions as described in [the Data
Import and Cleaning
vignette](https://docs.ropensci.org/pathviewr/articles/data-import-cleaning.html).
...
function, we’ll stop the all-in-one there by setting it and every step
after it to `FALSE` so we can take a closer look at how trajectories are
defined. Since all steps are set to `TRUE` by default, we don’t actually
need to list them in the function, just those we want to switch to
...
![](managing-frame-gaps_files/figure-html/cleanup_data-1.png)

### Inspecting the data

We’ve cleaned up our data and are ready to link individual data points
into continuous trajectories. Deciding exactly how to define a
...
`max_frame_gap = 1` will not allow any frame gaps–movement must be
continuous.
...
![](managing-frame-gaps_files/figure-html/max_frame_gap_1-1.png)
...
![](managing-frame-gaps_files/figure-html/max_frame_gap_5-1.png)
...
to clean out bits of data that do not span our region of interest. To
inspect each trajectory individually, we can then use the
...
![](managing-frame-gaps_files/figure-html/get_full_trajectories-1.png)
...
![](managing-frame-gaps_files/figure-html/get_full_trajectories-2.png)

### Visualize frame gap choice

`pathviewr` has several tools to help users decide what frame gap
allowances may be appropriate depending on their data and resolution
needs.
[`visualize_frame_gap_choice()`](https://docs.ropensci.org/pathviewr/reference/visualize_frame_gap_choice.md)
runs the
[`separate_trajectories()`](https://docs.ropensci.org/pathviewr/reference/separate_trajectories.md)
function over the same data set as many times as the user would like via
the `loop` argument, each time with a different `max_frame_gap`
allowance. Each loop represents an increase in the max frame gap value
of 1. For example the default of `loops = 20` will run
[`separate_trajectories()`](https://docs.ropensci.org/pathviewr/reference/separate_trajectories.md)
over the data set 20 times, with an increase in the `max_frame_gap`
argument of 1 each time.
...
![](managing-frame-gaps_files/figure-html/visualize_frame_gap_choice-1.png)
...
The output of
[`visualize_frame_gap_choice()`](https://docs.ropensci.org/pathviewr/reference/visualize_frame_gap_choice.md)
is a tibble and plot of the number of trajectories after running
[`separate_trajectories()`](https://docs.ropensci.org/pathviewr/reference/separate_trajectories.md)
with `max_frame_gap = 1`, `max_frame_gap = 2`, `max_frame_gap = 3`, etc.
We can see that as the frame gap allowance increases, more bits of data
are being linked into continuous trajectories and thus the total number
of trajectories decreases. The vertical line on the plot indicates the
“elbow” of the plot or the point at which counts of trajectories appear
to stabilize and increases in the `max_frame_gap` allowance no longer
effect the trajectory count very much.

### Autodetect

Setting `max_frame_gap = "autodetect"` rather than a numeric value uses
the “elbow” of the plots from
[`visualize_frame_gap_choice()`](https://docs.ropensci.org/pathviewr/reference/visualize_frame_gap_choice.md)
to guesstimate the best value(s) for `max_frame_gap`.

**In addition to automatically selecting the `max_frame_gap` depending
on the data, `autodetect` does so on a per subject basis rather than
applying the same allowable frame gap to all data in the data set since
frame gaps can vary between subjects.**

The cap on how high the `max_frame_gap` can go is defined as a
proportion of the capture frame rate and set by the
`frame_rate_proportion` argument, which defaults to `.10`.
...
![](managing-frame-gaps_files/figure-html/max_frame_gap_auto-1.png)

    #> For subject: device03, estimated best value for max_frame_gap: 2

![](managing-frame-gaps_files/figure-html/max_frame_gap_auto-2.png)

    #> For subject: device05, estimated best value for max_frame_gap: 2

![](managing-frame-gaps_files/figure-html/max_frame_gap_auto-3.png)
...
`FALSE`). `frame_gap_plotting = TRUE` will display the elbow plots for
