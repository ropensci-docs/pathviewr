...
`pathviewr_steps` that includes `"viewr"`) that combines all the rows of
the source viewr objects in `obj_list`. Metadata may not necessarily be
retained and therefore `attributes` should be used with caution.
